...
def cosent_loss(y_true, y_pred):
    ...
    """
    y_true, y_pred = y_true[:, 0], K.clip(y_pred[:, 0], -1e6, 1e6)
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if accuracy > self.best_accuracy:
            ...
            model.save_weights('%s.interact_cosent.weights' % task_name)
        print(
...
if __name__ == '__main__':
    ...
    model.load_weights('%s.interact_cosent.weights' % task_name)
